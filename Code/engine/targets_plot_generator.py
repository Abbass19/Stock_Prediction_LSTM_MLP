...
from Code.engine.preprocessing import get_dates_from_csv
from Code.engine.utilities import flatten_predictions, to_numpy
...
#A function added by me to make using the TPG functions easier.
def save_plots_to_default_folder(predicted_targets, actual_targets, dates, target, calculated_yield, folder_name):
    ...
    # Then enter "NoteBooks" inside that parent directory
    output_folder = os.path.join(parent_dir, "NoteBooks" , folder_name)
...
#The TPF function caller. An easy link between Data and TPG
# Acts as an API for all this class
def run_TPG_link(predictions: Union[list, np.ndarray],actual: Union[list, np.ndarray],
    target: str = "TARGET",folder_name: str = "TPG_Output"):
